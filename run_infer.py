...
################### Retrieving consensus results for an ensemble of models, if presented with multiple 
...
def mean_img_colors(image_path):
    ...
    # TODO: Separate all color channels into indiv dict entries
...
def _helper_img_color(img_file, img_dir):
    ...
    # WARNING: img_name reported without file extension 
    out_dict['img_name'] = os.path.splitext(img_file)[0]
    
    # dict returned where img_name == key and 
    # dict_w_name = {os.path.splitext(img_file)[0]: out_dict}
    
    return out_dict


def mean_img_colors_dir(img_dir):
    # TODO: More robust way of extracting image only files?
    # Some conflict with image files extracted in run_infer function?
    accepted_img_ext = ['.png','.tif', '.tiff', '.jpeg', '.jpg']
    img_files = os.listdir(img_dir)
    # retrieve files with 
    img_files = [img_file for img_file in img_files if any(ext in img_file for ext in accepted_img_ext)]
    ...
    out_list = [val for val in results]
...
def QA_infer(basedir, sep):
    ...
    input_dir = os.path.join(basedir, 'input')
    # After completion, report on image properties
    img_props = mean_img_colors_dir(input_dir)
    
    props_df = pd.DataFrame(img_props)
    props_df.to_csv(os.path.join(basedir, 'input_image_properties.tsv'), sep = '\t', index = False)

    # if gt dir exists, eval available masks
    if os.path.exists(os.path.join(basedir, 'gt')):
        print('Evaluating prediction against ground truth')
        eval_list = QA_eval(basedir)
        
        # combine image properties and eval
        eval_df = pd.DataFrame(eval_list)
        # output as tsv
        eval_df.to_csv(os.path.join(basedir, 'eval.tsv'), sep = '\t', index = False)
